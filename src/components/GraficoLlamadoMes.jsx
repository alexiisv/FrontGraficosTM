...
export default function GraficoLlamadosPorMes({
    data = [],
    titulo = "Llamados por mes y pabellón"
}) {
    ...
    // Obtener todos los pabellones únicos desde evento.pabellon
    const pabellonesUnicos = useMemo(() => {
        return [...new Set(data.map(d => d.evento.pabellon))];
    }, [data]);

    //Para que se inicialice todos los datos cargados
    useEffect(() => {
        setPabellonesSeleccionados(pabellonesUnicos);
    }, [pabellonesUnicos]);

    // console.log("datostimer", data)

    // Agrupar por mes y pabellón, solo para el color seleccionado
    const datosFiltrados = useMemo(() => {
        const conteo = {};

        data.forEach(d => {
            const e = d.evento;
            const mes = new Date(e.tiempoInicial).toISOString().slice(0, 7); // "YYYY-MM"
            ...
            if (!conteo[mes][e.pabellon]) conteo[mes][e.pabellon] = 0;
            conteo[mes][e.pabellon]++;
        });

        return Object.entries(conteo).map(([mes, valores]) => {
            const totalMes = Object.values(valores).reduce((sum, v) => sum + v, 0);
            return {
                mes,
                ...valores,
                total: totalMes
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-200 w-full">
                ...
                <div style={{ marginBottom: "1rem" }}>
                    <label>
                        ...
                        // className="bg-indigo-100 mt-1 ml-75 block w-full max-w-xs pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
                ...
                <div className="flex flex-col md:flex-row gap-6 items-start w-full">
                    <div className="w-full md:w-64 bg-white rounded-2xl shadow-lg p-2 border border-gray-200">
                        {/* <h3 className="text-base font-semibold text-gray-700 mb-3">Filtrar por pabellón</h3> */}
